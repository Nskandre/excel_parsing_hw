...
def make_report(log_file, report_template, report_output):
    ...
    _sorted_visits_of_browser = sorted_visits_of_browser(
    _visits_of_browsers)  # сортировка посещений браузеров в обратном порядке
    _raiting_of_browsers = raiting_of_browsers(_visits_of_browsers,
                                               _sorted_visits_of_browser)  # 7 самых популярных браузеров
    _res_browsers = res_browsers(_raiting_of_browsers,
                                 browsers)  # 7 самых популярных браузеров с посещениями по месяцам
    ...
    _sorted_buyings_of_goods = sorted_buyings_of_goods(
    _buyings_of_goods)  # сортировка товаров по покупкам в обратном порядке
